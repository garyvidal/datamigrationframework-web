...
import { generateJsonPreview, JsonPreviewResponse } from '@/services/ProjectService';
import { HighlightedJson } from '@/components/DocumentModelView/JsonPreview';
...
export default function GenerateJsonModal({ projectId, projectName, onClose }: GenerateJsonModalProps) {
    const [limit, setLimit] = useState(10);
    const [pendingLimit, setPendingLimit] = useState(10);
    const [loading, setLoading] = useState(false);
    const [result, setResult] = useState<JsonPreviewResponse | null>(null);
    ...
    const runPreview = useCallback(async (requestedLimit: number) => {
        ...
        setDocIndex(0);
        try {
            const data = await generateJsonPreview(projectId, requestedLimit);
            setResult(data);
            ...
            >
                ...
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-700 shrink-0">
                    <div className="flex items-center gap-3">
                        <span className="text-sm font-semibold text-gray-200">
                            Generate JSON — <span className="text-cyan-400">{projectName}</span>
                        </span>
                        {!loading && result && (
                ...
                <div className="flex items-center gap-3 px-4 py-2 border-b border-slate-800 shrink-0 bg-slate-900">
                    ...
                    <input
                        ...
                        onKeyDown={e => e.key === 'Enter' && runPreview(pendingLimit)}
                    ...
                    />
                    <button
                        onClick={() => runPreview(pendingLimit)}
                        disabled={loading}
                ...
                <div className="flex-1 overflow-auto p-4 min-h-0">
                    ...
                    {!loading && !error && docCount === 0 && (
                        <div className="flex items-center justify-center h-32 text-gray-500 text-sm">
                            No documents generated. Check that the project has a JSON document model mapping.
                        </div>
                    )}
                    {!loading && !error && currentDoc && (
                        <HighlightedJson json={currentDoc} />
                    )}
                </div>
            </div>
        </div>,
