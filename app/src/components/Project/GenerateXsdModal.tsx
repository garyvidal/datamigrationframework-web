// GenerateXsdModal — Portal modal for generating, viewing, copying, and downloading an XSD schema derived from the project's XML document mapping.
import React, { useCallback, useEffect, useState } from 'react';
import ReactDOM from 'react-dom';
import { FaCheck, FaCopy, FaSpinner, FaDownload, FaRedo } from 'react-icons/fa';
import { generateXsdSchema } from '@/services/ProjectService';
import { HighlightedXml } from '@/components/DocumentModelView/XmlPreview';

interface GenerateXsdModalProps {
    projectId: string;
    projectName: string;
    onClose: () => void;
}

export default function GenerateXsdModal({ projectId, projectName, onClose }: GenerateXsdModalProps) {
    ...
    const runGenerate = useCallback(async () => {
        ...
        setError(null);
        try {
            const xsd = await generateXsdSchema(projectId);
            setSchema(xsd);
        } catch (err) {
            ...
            >
                ...
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-700 shrink-0">
                    <span className="text-sm font-semibold text-gray-200">
                        XSD Schema — <span className="text-violet-400">{projectName}</span>
                    </span>
                    <div className="flex items-center gap-2">
                        ...
                        {schema && (
                            <button
                                ...
                                title={`Download as ${projectName}.xsd`}
                                className="flex items-center gap-1.5 px-2.5 py-1 text-xs rounded border border-violet-600 text-violet-400 hover:bg-violet-900/30 hover:text-violet-300 transition"
                            >
                                <FaDownload size={10} />
                                Download .xsd
                            </button>
                ...
                <div className="flex-1 overflow-auto p-4 min-h-0">
                    {loading && (
                        <div className="flex items-center justify-center h-32 text-gray-500 gap-2">
                            ...
                            <span className="text-sm">Generating schema…</span>
                        </div>
                    )}
                    {!loading && error && (
                    ...
                    {!loading && !error && !schema && (
                        <div className="flex items-center justify-center h-32 text-gray-500 text-sm">
                            No schema generated. Check that the project has an XML document mapping configured.
                        </div>
                    )}
                    {!loading && !error && schema && (
                        <HighlightedXml xml={schema} />
                    )}
                </div>
            </div>
        </div>,
